...
* @return
*/
bool FFEncoder::initEncoder() {
    if (isInited) {
        return true;
    }
    do {
        AVDictionary dictionary;
        AVDictionary *opt =  &dictionary;
        int ret;
 //       av_log_set_callback(ffmpeg_log);
        // 注册
        av_register_all();
        // 分配输出媒体上下文
        avformat_alloc_output_context2(&fmt_ctx, NULL, NULL, filePath);
        if (fmt_ctx == NULL) {
            break;
        }
        // 获取AVOutputFormat
        ...
        // 打开输出文件
        ret = avio_open(&(fmt_ctx->pb), filePath, AVIO_FLAG_READ_WRITE);
        if (ret < 0) {
 ...
 * @return
 */
bool FFEncoder::addStream(AVStream *ost, AVFormatContext *oc, AVCodec **codec,
 ...
 * @return
 */
bool FFEncoder::openAudio(AVCodec *codec, AVStream *ost, AVDictionary *opt_arg) {
 ...
 * @return
 */
AVFrame* FFEncoder::allocAudioFrame(int channels, enum AVSampleFormat sample_fmt,
 ...
 * @return
 */
bool FFEncoder::openVideo(AVCodec *codec, AVStream *ost, AVDictionary *opt_arg) {
    int ret;
    // 获取视频编码上下文
 ...
 * @return
 */
AVFrame* FFEncoder::allocVideoFrame(enum AVPixelFormat pix_fmt, int width, int height) {
    AVFrame *picture;
    int ret;
 ...
 * @return
 */
bool FFEncoder::videoEncode(uint8_t *data) {
    int ret;
    // 获取输出码流
    AVStream *ost = &video_st;
    AVCodecContext *context;
    AVFrame *frame;
    int got_frame = 0;
    // 获取视频编码上下文
 ...
 * @return
 */
bool FFEncoder::audioEncode(uint8_t *data, int len) {
    AVCodecContext *context;
    AVFrame *frame = NULL;
    int ret;
    int got_frame;
    int dst_nb_samples;
    AVStream *ost = &audio_st;
    // 获取源数据
 ...
 * @param ost
 */
void FFEncoder::closeStream(AVFormatContext *oc, AVStream *ost) {
    // 关闭编码上下文
    if (ost->enc != NULL) {
